// Dropdown Component - Tema destekli açılır menü (bugfix + theme + Text variants)
...
const Dropdown: React.FC<DropdownProps> = ({
  ...
  const computeListGeometry = (btn: { top: number; left: number; width: number; height: number }) => {
    const idealHeight = Math.min(
      MAX_LIST_HEIGHT,
      Math.max(ITEM_HEIGHT * Math.min(options.length, 6), ITEM_HEIGHT * 4)
    );
  ...
  const renderOption = ({ item }: { item: DropdownOption }) => {
    ...
    return (
      ...
      >
        <Text variant={isSelected ? 'accent' : 'primary'} size="medium">{item.flag}</Text>
        <View style={styles.optionTextContainer}>
          ...
          <Text variant="secondary" size="small">{item.nativeName}</Text>
        </View>
    ...
    <View variant="transparent" style={[styles.container, style] as any}>
      ...
      >
        <View style={styles.selectedContent}>
          {selectedOption ? (
            <>
              <Text variant="primary" size="medium">{selectedOption.flag}</Text>
